...
export const command: TextCommand = {
  ...
  run({ client, msg, args, content }) {
    ...
    if (!args.length && !msg.reference) {
      ...
      if (!emo) {
        fetch(`https://cdn.discordapp.com/emojis/${args[0]}.gif`).then(
          (resp) => {
            if (resp.status == 200) {
              emote.animated = true;
              msg.channel.send({ embeds: [embedIt(emote)] });
            } else {
              fetch(`https://cdn.discordapp.com/emojis/${args[0]}.png`).then(
                (resp) => {
                  if (resp.status == 200) {
      ...
      let emo =
        msg.client.emojis.cache.find((e) => e.name == args[0]) ||
      ...
      msg.channel.messages.fetch(msg.reference.messageId).then((m) => {
        ...
        if (!emoList.length) {
          msg.reply(
            'Reference messages dosent contain any resolvable emojies!',
          );
          return;
        }
        new embedPagination(
