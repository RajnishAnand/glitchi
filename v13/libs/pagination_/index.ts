import { CommandInteraction, Interaction, Message, MessageContextMenuInteraction, Snowflake, UserContextMenuInteraction } from 'discord.js';
import { select } from '..';
...
export default class pagination {
	...
	constructor(
		message: Message | CommandInteraction | UserContextMenuInteraction | MessageContextMenuInteraction,
		...
		//TODO : if ever fails set deferReply
			...
			if (typeof this.handler.value == 'string')
				message
					...
					.then((m0) => {
						if (!(m0 instanceof Message)) return new Error('Invalid instance if message');
						this.msg = m0;
						this.main();
					});
			else if (Array.isArray(argument))
	...
	private async main() {
		...
		if (this.handler.length > 2 || this.HandleHandler) await this.msg.react(this.buttons[0]);
		if (this.handler.length > 2 && !this.HandleHandler) await this.msg.react(this.buttons[1]);
		if (this.handler.length > 1 || this.HandleHandler) await this.msg.react(this.buttons[2]);
		if (this.handler.length > 1 || this.HandleHandler) await this.msg.react(this.buttons[3]);
