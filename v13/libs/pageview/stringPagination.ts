...
export class stringPagination {
	...
	declare refMsg: Message | CommandInteraction | UserContextMenuInteraction | MessageContextMenuInteraction;
	declare msg: Message;
	declare filter: (i: ButtonInteraction) => boolean;

	constructor(
		refMsg: Message | CommandInteraction | UserContextMenuInteraction | MessageContextMenuInteraction,
		text: string,
		...
		// filter
		const userSnowflake = this.refMsg instanceof Message ? this.refMsg.author.id : this.refMsg.user.id;
	...
	// initialisation
	async init() {
		await (this.refMsg instanceof Message ? this.refMsg.channel.send(this.value) : this.refMsg.reply({ ...this.value, fetchReply: true })).then(
			(m) => {
		...
		collector.on('collect', async (interaction) => {
			...
			switch (interaction.customId) {
				...
				case 'page':
					...
					break;
				case 'delete':
					interaction.deleteReply().catch(() => this.msg.delete().catch(() => {}));
					break;
			}
	...
	async goto(interaction: ButtonInteraction) {
		...
		interaction
			.awaitModalSubmit({
				filter: (x) => +x.fields.getTextInputValue('topage') > 0 && +x.fields.getTextInputValue('topage') < this.length + 1,
				time: 40000
			})
			.then((interaction) => {
