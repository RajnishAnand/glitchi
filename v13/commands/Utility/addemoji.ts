...
export const command: Command = {
  ...
  async run({ msg, args }) {
    ...
    const emojiNameOverride = args
      .find((e, i) => e.startsWith('--name=') && args.splice(i, 1))
      ?.replace('--name=', '');
        ...
        if (new RegExp(ereg).test(refMsg.content)) {
          ...
          emojiUrl = await select(msg, {
            ...
            options: matches.map((m) => ({
              label: m[2],
              value: `https://cdn.discordapp.com/emojis/${m[3]}.${
                m[1] ? 'gif' : 'png'
              }`,
              emoji: m[0],
            })),
    ...
    // from attachments
      ...
      (emojiName = args[0]), (emojiUrl = msg.attachments.first()?.url);
    }
