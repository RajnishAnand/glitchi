...
export const command: Command = {
	...
	async run({ msg, args }) {
		...
		function add() {
			...
			const command: ApplicationCommandDataResolvable | undefined = msg.client.slashCommands.get(args[0]);
		...
		async function remove() {
			...
			const command = await msg.guild!.commands.fetch({ cache: false }).then((l) => l.find((c) => c.name == args[0]));
		...
		function set() {
			...
			const commands: ApplicationCommandDataResolvable[] = msg.client.slashCommands
