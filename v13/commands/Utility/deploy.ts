...
export const command: Command = {
  ...
  async run({ msg, args }) {
    ...
    if (subCommand == 'list') list();
      ...
      return msg.reply(
        `Use \`${msg.client.config.prefix}help deploy\` to get help.`,
      );
        ...
        embeds: [
          {
            ...
            description: msg.client.slashCommands
              .map((c) => {
                return `${c.name} : ${
                  !c.type || c.type == 'CHAT_INPUT' ? c.description : c.type
                }`;
              })
              .join('\n'),
            timestamp: new Date(),
    ...
    function add() {
      ...
      msg
        ...
        .then(() => {
          msg.channel.send(
            `${msg.client.config.emojis.salute} Successfully added /${command.name} command.`,
          );
        })
        .catch((e) => {
          console.log(e);
    ...
    async function remove() {
      ...
      msg
        ...
        .then(() => {
          msg.channel.send(
            `${msg.client.config.emojis.salute} Successfully removed /${command.name} command.`,
          );
        })
        .catch((e) => {
          console.log(e);
    ...
    async function update() {
      ...
      try {
        ...
        msg.channel.send(
          msg.client.config.emojis.salute +
