...
export const command: Command= {
  ...
  run({msg,args,commandName}){
    ...
    //msg.delete();
    (msg.client.channels.cache.get(suggest?msg.client.config.channels.suggestion:msg.client.config.channels.feedback) as TextChannel).send({embeds : [embed]})
      .then((m) => {
        if(suggest){
      ...
      msg.channel.send(`Thank you for your ${suggest?'Suggestion':'Feedback'}! Your ${suggest?'Suggestion':'Feedback'} was successfully sent to support server.\😊`) 
    );
  }
}
