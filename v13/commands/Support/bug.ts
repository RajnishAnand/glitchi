...
export const command : Command ={
  ...
  async run({msg,args,content}){
    ...
    if(msg.reference&&msg.reference.messageId)await msg.channel.messages.fetch(msg.reference.messageId).then((m)=>{
        embed.fields?.push({
          name : '🌡| Referenced Message :',
          value:`­     └⊸ ID : [${msg.reference?.messageId}](https://discord.com/channels/${msg.guild?.id}/${msg.channel.id}/${msg.reference?.messageId})\n`+(m.embeds.length?'```json'+`\n${JSON.stringify(m.embeds).replace(/```/g,'`­``')}}`+'```\n': `>>> ${m.content.substring(0,500)}`),
          inline:false
        })
    });
   ...
   (msg.client.channels.cache.get(msg.client.config.channels.bugReport) as TextChannel).send({embeds:[embed]}).then((m)=>{Promise.all([
      m.react(msg.client.config.emojis.thumbsup),
      m.react(msg.client.config.emojis.thumbsdown),
      m.react('🪛'),
     ])})
   msg.channel.send(msg.client.config.emojis.sneak+' Sorry for your inconvenience. Bug was successfully reported to support server and will be fixed soon!');
  }
}
