...
async function run({ msg, args, content }: CommandArgument) {
	...
	const channel = await msg.client.channels.fetch(id).catch((err: any) => msg.reply(err.message));

	if (!channel || !(channel instanceof TextChannel)) return msg.reply('Channel is not a TextChannel!');
	...
	if (msg.channel.id in msg.client.config.block) return msg.reply('This channel is already connected to a channel!');
