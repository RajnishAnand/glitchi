...
function embedify(data: rockYouResponse) {
	if (!('password' in data)) {
		return new MessageEmbed({
			...
			description: `\`\`\`js\n{\n  code: ${data.code}\n  message: "${'data' in data ? data.data : data?.message}"\n} \`\`\``
		});
	}
	return new MessageEmbed({
		title: 'PassCheck',
		...
		image: {
			url: data.rocked ? 'https://c.tenor.com/bzVhf9TfRhoAAAAM/throw-rock.gif' : ''
		},
		description: `__Password:__ \`${data.password}\`\n└╼ is ${data.rocked ? '' : 'not'} on the dictionary attack list.\n> ${data.data}`
	});
}
