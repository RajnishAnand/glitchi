...
export const event: Event = {
	...
	async execute(client, msg: ExtendMessage) {
		...
		else if (!msg.content.toLowerCase().startsWith(client.config.prefix) || !msg.content) return;
		...
		try {
			...
			// and is not dev
			if ((command.devOnly || false) == true && (msg.author.id != client.config.ownerId) == true) {
				return;
			}
