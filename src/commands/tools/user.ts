        ...
        msg.client.users.fetch(id)
          .then((u:any)=>{
            msg.guild.members.fetch(u.id)
              .then((gu:any)=>send(msg,u,gu))
              .catch((err:any)=>send(msg,u));
          })
...
function send(msg:any,user:any=false,member:any=false){
  let embed : any = {};
  if(user){
    embed = {
      color : '#00bfff',
  ...
  if(member){
    ...
    embed.fields.push({
      ...
      name : `🪃| Roles [${member
        .roles.cache.size-1}]: `,
      value :(member.roles.cache.size-1)?member.roles.cache.filter((r:any)=>r.id!=msg.guild.id).map((r:any)=>r).join(", "):'` NONE `',
    });
    ...
    if(permKeys.includes('ADMINISTRATOR')){
      ...
      userKeyPerms = permKeys
        .filter((perm:string)=>keyPerms.includes(perm))
        .join(', ');
    }
    else{
