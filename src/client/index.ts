...
import { Zlink  } from './interface/zlinks';
...
export default class ExtendClient extends Client {
  ...
  public zLinkCommands: Collection<string, Zlink> = new Collection();
  ...
  async loadZlinkCommands(){
    const zLinkCommandsPath = path.join(
      __dirname,
      "..",
      "commands/zlinks"
    );

    const zlinkCommandFiles = readdirSync(zLinkCommandsPath).filter(
      (f) => f.endsWith(".js")
    );

    zlinkCommandFiles.forEach(async (filePath)=>{
      const zLinkCommand : Zlink = 
        require(`${zLinkCommandsPath}/${filePath}`).zlink;
