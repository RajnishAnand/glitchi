"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.command = void 0;
const util_1 = require("util");
const content_type_1 = require("content-type");
const _libs_1 = require("#libs");
const cbparser_1 = require("cbparser");
exports.command = {
    name: 'fetch',
    description: 'To fetch any URL .',
    argsHelp: ['<url>', '?<--get||--post||--headers>', '?<post_text> <?headers>'],
    args: true,
    examples: ['http://example.com/'],
    run,
};
const types = [
    'text/plain',
    'application/xml',
    'application/json',
    'application/x-www-form-urlencoded',
];
async function run({ msg, args, content }) {
    let response = '';
    let headers = {};
    let title;
    if (!args[1] || !args[1].startsWith('-'))
        args.splice(1, 0, '-g');
    if (args[1].toLowerCase() == '-g' || args[1].toLowerCase() == '--get') {
        let [head] = (0, cbparser_1.CBParser)(content().replace(/[­ ]/g, ''));
        if (head) {
            try {
                headers = JSON.parse(head.code);
            }
            catch (err) {
                return msg.reply('Error while parsing Headers:' + err.message);
            }
        }
        [response, title] = await GET(args[0], headers);
    }
    else if (args[1].toLowerCase() == '-p' ||
        args[1].toLowerCase() == '--post') {
        let [{ code, lang }, head] = (0, cbparser_1.CBParser)(content().replace(/[­ ]/g, ''));
        //post body requirement
        if (!code) {
            return msg.reply('Please also include body/text you wanna POST in a codeBlock !');
        }
        // headers
        if (head) {
            try {
                headers = JSON.parse(head.code);
            }
            catch (err) {
                return msg.reply('Error while parsing Headers:' + err.message);
            }
        }
        else if (lang == 'json') {
            headers = { 'Content-Type': 'application/json' };
            try {
                JSON.parse(code);
            }
            catch (err) {
                return msg.reply(err.message);
            }
        }
        else if (!lang || !types.includes(lang)) {
            try {
                headers = {
                    'Content-Type': await (0, _libs_1.select)(msg, {
                        content: 'Please pick a supported content type:',
                        title: 'Select content-type',
                        options: [
                            ...types.map((z) => {
                                return {
                                    label: z,
                                    description: `set request body format as ${z}`,
                                    value: z,
                                };
                            }),
                            {
                                label: 'Other',
                                description: 'custom content type',
                                value: 'none',
                            },
                        ],
                    }),
                };
                if (headers['Content-Type'] == 'none') {
                    const ans = await (0, _libs_1.ask)(msg, 'Enter your custom content type :');
                    if (!ans)
                        return msg.channel.send('Time Out! Command Cancelled.');
                    lang = ans;
                }
            }
            catch (err) {
                return msg.reply(err.message);
            }
        }
        [response, title] = await POST(args[0], code, headers);
    }
    else if (args[1].toLowerCase() == '-h' ||
        args[1].toLowerCase() == '--headers') {
        [response, title] = await Headers(args[0]);
    }
    else {
        return msg.reply(`*__${args[1].replace(/([\*\`\~\_])/g, '\\$1')}__* isn't a valid flag. Currently it only supports :\n   • \`[ -p ]\` or \`[ --post ]\` : POST REQUEST\n   • \`[ -g ]\` or \`[ --get ]\` : GET REQUEST (Default)\n   • \`[ -h ]\` or \`[ --headers ]\` : GET HEADERS`);
    } //`
    //type = type.includes('+')?type.split("+")[0]:type.split("/")[1]
    let langGuess = (0, content_type_1.parse)(title ?? 'application/error').type.split('/')[1];
    if (langGuess == 'json') {
        try {
            response = (0, util_1.inspect)(JSON.parse(response), { depth: Infinity });
            langGuess = 'js';
        }
        catch (err) { }
    }
    (0, _libs_1.createStringPagination)(msg, response, {
        decoration: { lang: langGuess, title },
    });
}
/////////////////////////////////////////////////
/**fetch HEADERS*/
async function Headers(url) {
    const response = await fetch(url)
        .then((resp) => resp.headers)
        .then((obj) => (0, util_1.inspect)(obj))
        .catch((err) => err.message);
    return [response, 'application/javascript'];
}
/**fetch GET request*/
async function GET(url, headers) {
    const controller = new AbortController();
    setTimeout(() => controller.abort(), 6000);
    const response = await fetch(url, {
        signal: controller.signal,
        headers,
    })
        .then(async (r) => [await r.text(), r.headers.get('content-type')])
        .catch((err) => {
        if (err.type == 'aborted')
            //warn if time exceeds
            return [
                'fetch request aborted! exceeds timelimit of 6000ms',
                'text/warning',
            ];
        return [err.message, 'text/plain'];
    });
    return response;
}
/**fetch POST request*/
async function POST(url, data, headers) {
    const response = await fetch(url, {
        method: 'POST', // *GET, POST, PUT, DELETE,
        headers: headers,
        redirect: 'follow',
        body: data,
    })
        .then(async (r) => [await r.text(), r.headers.get('content-type')])
        .catch((err) => {
        if (err.type == 'aborted')
            //warn if time exceeds
            return [
                'fetch request aborted! exceeds timelimit of 6000ms.',
                'text/warning',
            ];
        return [err.message, 'text/plain'];
    });
    return response;
}
