...
exports.command = {
    ...
    async run({ client, msg, args }) {
        ...
        if (!msg.reference?.messageId && !args.length) {
            msg.reply(client.config.emojis.aha + ' Looks like you forgot to enter Message Id');
            return;
        }
        ...
        //  resolve channel & messageID if in args
            ...
            messageID = args[1];
            if (!/^\d+$/.test(chID))
                return msg.reply(`unable to resolve \`${args[0]}\` as channel.`);
            if (!/^\d+$/.test(messageID))
            ...
            if (!channel)
                return msg.reply('unable to resolve channel.');
        }
        // resolve message if its from same channel
        else {
            if (!/^\d+$/.test(args[0]))
                return msg.reply(`unable to resolve \`${args[0]}\` as messageID`);
            messageID = args[0];
            channel = msg.channel;
        }
        ...
        if (messages) {
            messages.fetch(messageID).then((m0) => {
                if (m0.content.length)
                    (0, _libs_1.createStringPagination)(msg, m0.content);
                else
                    msg.reply({
