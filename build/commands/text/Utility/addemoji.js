...
exports.command = {
    ...
    async run({ msg, args, ref }) {
        ...
        // if all arguments
            ...
            emojiName = args[0];
            if (new RegExp(ureg).test(args[1]))
                emojiUrl = args[1];
            else if (new RegExp(ereg).test(args[1]))
                emojiUrl = emojiToUrl(args[1]);
            else
                return msg
                    .reply({
        ...
        // all from reference message
            ...
            if (new RegExp(ureg).test(referance.content)) {
                ...
                if (matches.length > 1)
                    emojiUrl = await (0, _libs_1.select)(msg, {
                        title: 'Select URL',
            ...
            // from ref emojis
                ...
                if (matches.length > 1)
                    ...
                    emojiUrl = emojiToUrl(matches[0]);
            }
            // feom ref attachment
            else if (referance.attachments.size)
                emojiUrl = referance.attachments.first()?.url;
            // from ref imo command
            else if (referance.embeds[0]?.url)
                emojiUrl = referance.embeds[0].url;
            if (emojiUrl == undefined)
                return msg
                    .reply({
                    content: 'Unable to find any appropriate URL, Emojis or attachement in the reference message.',
                })
                    .then((m) => (0, _libs_1.attachDeletable)(m, msg.author.id));
        }
        else
