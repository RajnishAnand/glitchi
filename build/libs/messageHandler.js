...
async function messageHandler(client, msg) {
    if (msg.author.bot || msg.channel.type == discord_js_1.ChannelType.DM || !msg.guild)
        return;
    ...
    if (process.env.PROTECTED) {
        const owner = await msg.client.guilds
            ...
            .then((g) => g.ownerId === msg.author.id &&
    ...
    const command = client.textCommands.get(commandName) || client.aliases.get(commandName);
    ...
    if (command.requiredPerms) {
        const myPerms = msg.channel.permissionsFor(client.user.id);
    ...
    // and is not dev
    if ((command.ownerOnly || false) == true &&
    ...
    // in args required commands
        ...
        return {
            command,
            error: 'ARGSMISSING',
            message: `${command.argsHelp ? '```\n' + command.argsHelp.join(' ') + ' ``` ' : ''}Command : \` ${command.name} \` requires argument! ${client.config.emojis.sneak}.\nType \`${client.config.prefix}help ${command.name}\` to get help on it.`,
        };
    }
    // execute command
    else {
        ...
        return {
            args: { client, msg, args, content, commandName, ref },
            command,
            // error:this.err
        };
    }
